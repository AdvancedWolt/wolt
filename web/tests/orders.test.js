...
test('GET /api/orders returns orders for the authenticated user', async () => {
    ...
    // Create two orders
    await request('POST', '/api/orders', { restaurantId, items: ['item-1'] }, headers);
    await request('POST', '/api/orders', { restaurantId, items: ['item-2'] }, headers);
...
test('GET /api/orders does not return other users orders', async () => {
    ...
    await request('POST', '/api/orders', { restaurantId, items: ['alice-item'] }, alice.headers);
...
test('GET /api/orders/:id returns order details', async () => {
    ...
    assert.deepStrictEqual(res.json.items, ['burger', 'fries']);
...
test('PATCH /api/orders/:id updates order items -> 204', async () => {
    ...
    const fetched = await request('GET', `/api/orders/${orderId}`, undefined, headers);
    assert.deepStrictEqual(fetched.json.items, ['new-item-1', 'new-item-2']);
});
...
test('full order lifecycle: create -> read -> update -> delete', async () => {
    ...
    assert.deepStrictEqual(read.json.items, ['pizza']);
    ...
    assert.deepStrictEqual(readUpdated.json.items, ['pizza', 'cola']);
